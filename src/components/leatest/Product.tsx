...
const Product = ({ item }) => {
  ...
  return (
    <li className="flex flex-col">
      <div
        className={`relative flex items-center w-full h-72 bg-bg_product ${isShow && currentActive === item ? 'bg-white transition-all duration-300 ease-in-out' : ''}`}
        onMouseEnter={() => handleShow(item.id)}
        onMouseLeave={handleHidden}>
        <img className="w-56 h-60 m-auto object-cover" src={item.images[0]} alt={item.title} />

        <div
          className={`product__visible ${isShow && currentActive === item ? 'visible' : ''}`}>
